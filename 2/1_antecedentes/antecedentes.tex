...
\subsection{Food Sales Prediction Strategy Using Machine Learning and Deep Learning Algorithms \citep{foodsales2023}}
\subsubsection{Planteamiento del Problema y objetivo}

El estudio desarrollado por diversos autores abordó el problema de la predicción de ventas de productos alimenticios mediante técnicas de \textit{forecasting}, \textit{Machine Learning} y \textit{Deep Learning}. Los investigadores señalan que las empresas dedicadas a la comercialización de alimentos enfrentan dificultades para estimar con precisión la demanda futura, lo cual puede ocasionar pérdidas económicas debido al desperdicio de productos perecibles, exceso de inventario o escasez de productos para los consumidores.

El objetivo principal de la investigación fue desarrollar una estrategia de predicción de ventas que permitiera identificar los algoritmos con mayor precisión y eficiencia para el pronóstico de demanda alimentaria. Para ello, se analizaron distintos modelos predictivos basados en técnicas de inteligencia artificial, con el propósito de optimizar la gestión de inventarios y mejorar la toma de decisiones empresariales.

Asimismo, el estudio buscó determinar el impacto de la correlación entre variables dentro de diferentes conjuntos de datos, evaluando cómo dichas relaciones afectan el desempeño de los modelos predictivos. Como aplicación final, los autores realizaron predicciones futuras sobre cantidades de productos alimenticios utilizando información histórica de ventas.


\subsubsection{Base de datos de los autores}

La investigación utilizó cuatro conjuntos de datos relacionados con ventas de alimentos y productos comerciales. Los tres primeros \textit{datasets} fueron empleados para comparar el desempeño de diversos algoritmos de predicción, mientras que el cuarto \textit{dataset} fue utilizado para realizar pruebas reales de \textit{forecasting} sobre producción y ventas futuras.

El primer conjunto de datos provenía de la plataforma Alibaba Tianchi y contenía información de ventas de alimentos correspondientes al año 2019. Este \textit{dataset} estaba conformado por aproximadamente 1000 registros y 15 variables relacionadas con categorías de productos, cantidades y precios.

El segundo \textit{dataset} fue obtenido desde Kaggle y contenía información de ventas de alimentos de diez tiendas ubicadas en diferentes zonas geográficas. Este conjunto contaba con aproximadamente 8523 registros y 12 atributos asociados a precios, tipos de productos y establecimientos.

El tercer conjunto de datos también provenía de Kaggle y estaba relacionado con ventas de diferentes categorías y subcategorías de alimentos en distintas ciudades, incluyendo alrededor de 9995 registros y 10 atributos.

Finalmente, el cuarto \textit{dataset} correspondía a ventas de dátiles en Iraq entre los años 2002 y 2020, incluyendo variables como cantidades producidas, precios, tipos de productos y número de árboles de palma. Este conjunto de datos fue utilizado para validar el modelo propuesto y realizar predicciones futuras de producción y ventas.

\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{2/1_antecedentes/img/Dt_foodsales.jpg}
        \caption{Base de datos de los autores}
        \label{fig10}
    \end{center}
\end{figure}
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{2/1_antecedentes/img/M_foodsales.jpg}
        \caption{Metodología empleada por los autores}
        \label{fig11}
    \end{center}
\end{figure}
...
\begin{itemize}
    ...
    La metodología incluyó la aplicación y comparación de múltiples algoritmos de \textit{Machine Learning} y \textit{Deep Learning} orientados a tareas de regresión y \textit{forecasting}. Entre los modelos utilizados se encuentran \textit{Linear Regression}, \textit{Decision Tree}, \textit{Random Forest}, \textit{K-Nearest Neighbors}, \textit{Gradient Boosting Regression}, \textit{Bayesian Ridge Regression}, \textit{Support Vector Regression} y modelos de \textit{Deep Learning} como \textit{LSTM}.

    \item \textbf{Etapas del proceso:}

    El proceso metodológico comenzó con la carga de múltiples \textit{datasets} relacionados con ventas de alimentos. Posteriormente, se realizó la visualización de datos mediante gráficos y diagramas para comprender el comportamiento de las variables. Luego se efectuó el preprocesamiento de datos, incluyendo limpieza, normalización y análisis de correlación entre variables mediante mapas de calor (\textit{Heatmaps}). Finalmente, los \textit{datasets} fueron ingresados a trece algoritmos distintos para comparar su rendimiento predictivo utilizando métricas de evaluación.

    \item \textbf{Herramientas y tecnologías utilizadas:}

    Los autores utilizaron el lenguaje de programación Python junto con librerías especializadas en ciencia de datos y \textit{Machine Learning}. Entre las herramientas empleadas destacan \texttt{pandas} y \texttt{NumPy} para el procesamiento de datos, \texttt{Seaborn} y \texttt{Matplotlib} para visualización gráfica, y bibliotecas de aprendizaje automático para entrenamiento y evaluación de modelos predictivos.

    \item \textbf{Procedimientos implementados:}

    Los procedimientos incluyeron análisis exploratorio de datos, evaluación de correlación entre variables, entrenamiento independiente de cada modelo y comparación de resultados mediante métricas de error. Los autores realizaron diferentes pruebas sobre \textit{datasets} con distintos niveles de correlación para determinar qué algoritmos ofrecían mejores resultados bajo determinadas condiciones de los datos.

    \item \textbf{Evaluación de resultados:}

    Para medir el desempeño de los modelos predictivos, se utilizaron métricas de evaluación como RMSE (\textit{Root Mean Square Error}), MAE (\textit{Mean Absolute Error}), MSE (\textit{Mean Squared Error}) y otros indicadores de precisión. Estas métricas permitieron comparar objetivamente la eficiencia de cada algoritmo en tareas de \textit{forecasting} y predicción de ventas.

\end{itemize}

\subsubsection{Resultados obtenidos}

Los resultados demostraron que los modelos basados en técnicas de \textit{Machine Learning} y \textit{Deep Learning} obtuvieron una alta precisión en la predicción de ventas y demanda alimentaria. Los autores concluyeron que algoritmos \textit{ensemble} como \textit{Random Forest} y \textit{Gradient Boosting Regression} presentaron un desempeño superior frente a modelos tradicionales en diversos escenarios.

Asimismo, los modelos de \textit{Deep Learning} como \textit{LSTM} mostraron una gran capacidad para capturar patrones temporales complejos y realizar predicciones de largo plazo con mayor precisión.

Finalmente, el estudio concluyó que la combinación de técnicas de \textit{forecasting}, \textit{Machine Learning} y análisis de variables permite mejorar significativamente la predicción de demanda, optimizando la gestión de inventarios y reduciendo pérdidas económicas en empresas del sector alimentario. Este antecedente guarda relación con la presente investigación debido a que ambos trabajos buscan desarrollar modelos predictivos basados en datos históricos para estimar la demanda futura mediante técnicas de aprendizaje automático.


\begin{figure}[H]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{2/1_antecedentes/img/R_foodsales.jpg}
        \caption{Resultados obtenidos en el antecedente}
        \label{fig12}
    \end{center}
\end{figure}


\subsection{Modelo predictivo de ventas usando aprendizaje automático (Machine Learning) para pronosticar las ventas diarias de una empresa}

\subsubsection{Planteamiento del problema y objetivo}

La tesis desarrollada por Baron Jaramillo (2024), en la Universidad Nacional Abierta y a Distancia -- UNAD de Colombia, aborda el problema de la predicción de ventas diarias en una empresa comercial mediante el uso de técnicas de aprendizaje automático. El estudio parte de la necesidad de mejorar la capacidad de las organizaciones para anticipar el comportamiento futuro de sus ventas, considerando que una estimación imprecisa puede afectar la planificación de inventarios, la asignación de recursos, la gestión operativa y la toma de decisiones estratégicas.

El problema central identificado por el autor se relaciona con las limitaciones que presentan los métodos tradicionales de pronóstico cuando deben capturar patrones complejos de series temporales, especialmente aquellos asociados a estacionalidad, días festivos, vacaciones y variaciones propias del comportamiento comercial. Frente a ello, se plantea el uso de modelos de Machine Learning como una alternativa para mejorar la precisión de los pronósticos y generar información útil para la gestión empresarial.

El objetivo general de la investigación fue evaluar la efectividad de un modelo de Machine Learning implementado mediante el algoritmo Facebook Prophet para la predicción de ventas diarias en una empresa del sector comercial. Asimismo, el trabajo buscó comparar dicho modelo con métodos tradicionales de series temporales, como ARIMA y SARIMA, empleando métricas de evaluación que permitieran determinar el nivel de precisión y ajuste de cada modelo.

Este antecedente guarda relación con la presente investigación, debido a que ambos estudios emplean técnicas de predicción basadas en datos históricos de ventas para apoyar la toma de decisiones empresariales. Además, ambos trabajos consideran la comparación de modelos predictivos y el uso de métricas cuantitativas como parte fundamental para determinar qué enfoque ofrece mejores resultados en términos de precisión y aplicabilidad.

\subsubsection{Base de datos de los autores}

La investigación utilizó una base de datos histórica de ventas diarias de una empresa comercial. Según el autor, el conjunto de datos empleado permitió simular el rol de un científico de datos dentro del área de ventas, con el propósito de predecir ventas futuras a partir de información histórica. La base de datos consideró registros asociados a tiendas comerciales, fechas y ventas, permitiendo estructurar una serie temporal orientada al análisis predictivo.

Dentro del proceso de análisis, el autor examinó la información disponible para identificar el número de registros, los tipos de datos, la presencia de valores nulos y las características generales del conjunto de datos. Posteriormente, se realizaron gráficos exploratorios para observar el comportamiento histórico de las ventas, así como patrones relacionados con el día, mes y año. Este procedimiento permitió identificar tendencias, variaciones y posibles comportamientos estacionales dentro de la serie de ventas.

La base de datos cumplió un papel fundamental en la investigación, ya que permitió entrenar y evaluar los modelos predictivos. Asimismo, la información histórica fue transformada y adaptada para cumplir con los requerimientos del algoritmo Facebook Prophet, el cual exige que las columnas de fecha y ventas sean renombradas como \textit{ds} y \textit{y}, respectivamente.

\subsubsection{Metodología empleada por los autores}

La metodología empleada por Baron Jaramillo (2024) se basó en el enfoque CRISP-DM, el cual proporciona una secuencia estructurada para el desarrollo de proyectos de minería de datos y Machine Learning. El proceso metodológico incluyó etapas de recolección, comprensión, preprocesamiento, modelamiento, evaluación y comparación de resultados.

En primer lugar, se realizó la recolección y carga del conjunto de datos históricos de ventas. Posteriormente, se efectuó una revisión de la estructura de la base de datos, identificando los campos disponibles, tipos de datos, número de registros y existencia de valores nulos. Esta fase permitió determinar si la información era adecuada para el análisis de series temporales y para el entrenamiento de modelos predictivos.

En segundo lugar, se desarrolló el análisis exploratorio de datos, mediante estadísticas descriptivas, mapas de calor, histogramas y gráficos de comportamiento histórico. A partir de este análisis, se identificaron patrones temporales en las ventas, tales como variaciones por día, mes y año. También se observó la existencia de comportamientos particulares en determinados periodos, como meses con mayores niveles de venta y tendencias decrecientes en ciertos tramos de la serie.

En tercer lugar, se implementó el modelo Facebook Prophet. Para ello, el autor preparó la base de datos renombrando las columnas principales de fecha y ventas, aplicó transformaciones sobre la serie temporal y entrenó el modelo mediante el método \textit{fit()}. Luego, se generó un pronóstico a treinta días utilizando la función \textit{make\_future\_dataframe}, con el fin de estimar el comportamiento futuro de las ventas.

Posteriormente, el autor incorporó días feriados y periodos de vacaciones dentro del modelo Prophet, con el objetivo de evaluar si estos factores mejoraban la precisión del pronóstico. Esta etapa permitió analizar la influencia de eventos especiales en la predicción de ventas y observar si su inclusión generaba cambios significativos en las métricas de evaluación.

Finalmente, se desarrollaron modelos tradicionales de comparación, específicamente ARIMA y SARIMA. Estos modelos fueron implementados para contrastar su rendimiento frente al modelo Facebook Prophet. La comparación se realizó utilizando métricas como el Error Cuadrático Medio (MSE), la Raíz del Error Cuadrático Medio (RMSE), el Error Absoluto Medio (MAE), el Error Porcentual Absoluto Medio (MAPE) y el coeficiente de determinación R\textsuperscript{2}.

\subsubsection{Resultados obtenidos}

Los resultados obtenidos por Baron Jaramillo (2024) evidenciaron diferencias importantes entre los modelos evaluados. En la comparación general, el modelo ARIMA obtuvo los menores valores en MSE, RMSE y MAE, lo que indicó un mejor desempeño en términos de error absoluto y cuadrático. Por otro lado, los modelos Facebook Prophet, tanto con feriados como sin feriados, presentaron mejores resultados en MAPE y en el coeficiente de determinación R\textsuperscript{2}.

En el caso de Facebook Prophet sin feriados ni vacaciones, el modelo obtuvo un MSE de 0.042068, RMSE de 0.205106, MAE de 0.159263, MAPE de 0.018904 y R\textsuperscript{2} de 0.261623. Por su parte, Facebook Prophet con feriados y vacaciones obtuvo un MSE de 0.042112, RMSE de 0.205211, MAE de 0.159111, MAPE de 0.018881 y R\textsuperscript{2} de 0.265667. Estos resultados mostraron que la inclusión de feriados y vacaciones produjo una mejora leve en algunas métricas, aunque no representó un cambio significativo en el rendimiento general del modelo.

El modelo ARIMA presentó un MSE de 0.026233, RMSE de 0.161967, MAE de 0.136773, MAPE de 1.595034 y R\textsuperscript{2} de 0.006082. Estos valores indicaron que ARIMA tuvo un mejor desempeño en errores absolutos y cuadráticos, pero menor capacidad explicativa respecto a los modelos Prophet. En cambio, SARIMA presentó un desempeño inferior, con errores significativamente más altos, especialmente en MSE, RMSE, MAE y MAPE.

Como conclusión, el autor señala que la elección del modelo depende del criterio de evaluación priorizado. Si el objetivo principal es reducir el error absoluto y cuadrático, ARIMA resulta más conveniente. Sin embargo, si se prioriza la precisión relativa porcentual y el ajuste general del modelo, Facebook Prophet presenta mejores condiciones. Este antecedente resulta relevante para la presente investigación porque demuestra la importancia de comparar modelos predictivos mediante diferentes métricas y de considerar no solo el menor error numérico, sino también la aplicabilidad del modelo dentro del contexto empresarial.
% \subsubsection{Planteamiento del Problema y objetivo}
% \subsubsection{Base de datos de los autores}
% \subsubsection{Metodología empleada por los autores}
% \subsubsection{Resultados obtenidos}
