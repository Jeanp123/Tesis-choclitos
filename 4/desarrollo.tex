...
La adquisición de datos consistió en recopilar la información histórica de ventas de la empresa, inicialmente almacenada en archivos externos como hojas de cálculo o archivos CSV. Para el análisis predictivo se utilizaron principalmente las variables \textbf{FECHA}, \textbf{Sede}, \textbf{FAMILIA} y \textbf{ventas\_totales}, ya que estas permiten construir la serie temporal, segmentar la información por unidad operativa y definir la variable que será pronosticada.

Posteriormente, los datos fueron integrados en \textbf{Supabase}, una base de datos en la nube que permitió centralizar la información y mejorar su disponibilidad. Además, se organizaron tablas de usuarios y ventas, lo que permitió implementar un control de acceso por sede. Así, cada usuario puede visualizar únicamente los datos correspondientes a su sede, haciendo que el análisis y la predicción sean más específicos y seguros.
...
Después de ordenar los datos cronológicamente, se realizó la partición del conjunto de datos en entrenamiento y prueba. Esta división se hizo respetando el orden temporal, debido a que en los modelos de series de tiempo no es correcto mezclar los datos aleatoriamente, ya que el objetivo es predecir valores futuros a partir de información pasada.

El 80 \% de los datos fue utilizado para entrenar los modelos, mientras que el 20 \% restante fue destinado a la evaluación. Esta partición permitió simular un escenario real, donde el sistema aprende del historial de ventas y luego intenta predecir datos posteriores. De esta manera, se evaluó si el modelo podía anticipar correctamente el comportamiento futuro de la demanda.


\subsection{Preprocesamiento de datos}

Durante la etapa de preprocesamiento se aplicaron diversas técnicas orientadas a preparar, limpiar y optimizar la información antes del entrenamiento de los modelos predictivos. En primer lugar, se realizó la limpieza de datos, normalizando los nombres de las columnas, corrigiendo formatos e inconsistencias, eliminando registros duplicados e inválidos y tratando los valores nulos presentes en la base de datos histórica de ventas. Asimismo, la columna \textit{FECHA} fue convertida a formato de fecha y la variable \textit{ventas\_totales} a formato numérico, permitiendo su correcto análisis en Python.

Posteriormente, los datos fueron ordenados cronológicamente y filtrados según la sede y la familia de productos seleccionada. Además, se efectuó el agrupamiento de productos por familias, especialmente en la categoría de verduras, con el propósito de consolidar la información comercial, facilitar el análisis del comportamiento de la demanda y reducir la complejidad de los datos.

De igual manera, se realizó la ingeniería y transformación de variables temporales mediante la extracción de atributos relevantes de la fecha, tales como año, mes, día y día de la semana, incluyendo también un indicador de fin de semana. Estas variables permitieron identificar patrones de estacionalidad y tendencias en las ventas.

Adicionalmente, se implementó la generación de variables derivadas o \textit{lag features}, utilizando información de ventas de períodos anteriores mediante rezagos de 1, 7, 14 y 30 días, así como medias móviles de 7 y 30 días. Esto permitió que los modelos identificaran patrones diarios, semanales, quincenales y mensuales en la demanda, mejorando así su capacidad predictiva.

Finalmente, se efectuó el tratamiento de valores atípicos (\textit{outliers}) para disminuir el impacto de registros anormales que pudieran afectar la precisión del modelo, y se aplicaron técnicas de normalización sobre las variables numéricas con el fin de estandarizar los datos y mejorar el desempeño de los algoritmos de aprendizaje automático utilizados en la investigación.
...
Para el desarrollo de la solución se evaluaron diferentes modelos de \textit{machine learning} y series temporales, con el objetivo de identificar cuál ofrecía mejores resultados en la predicción de ventas. Entre los modelos utilizados se consideraron Regresión Lineal, Ridge, Lasso, \textit{Decision Tree}, \textit{Random Forest}, \textit{Extra Trees}, \textit{Gradient Boosting}, KNN y SVR.

Además, se incluyeron modelos orientados a series temporales como ARIMA, SARIMA y Prophet, los cuales permiten capturar patrones históricos, tendencias y estacionalidades. Todos los modelos fueron entrenados usando las mismas variables predictoras y la misma variable objetivo, \textit{ventas\_totales}, para asegurar una comparación justa entre los algoritmos.
...
La evaluación de los modelos se realizó comparando las predicciones generadas con los valores reales del conjunto de prueba. Para medir el desempeño se utilizaron métricas como MAE, MSE, RMSE y R\textsuperscript{2}. Estas métricas permitieron analizar el error promedio, penalizar errores grandes, interpretar el error en la misma unidad de ventas y conocer qué tan bien el modelo explica la variabilidad de los datos.

Con estos resultados se elaboró una comparación entre los modelos entrenados. El modelo ganador fue seleccionado considerando su desempeño general, es decir, buscando el menor nivel de error y una buena capacidad explicativa. Luego, el modelo con mejor rendimiento fue reentrenado con toda la información disponible para generar el pronóstico final.
...
Después de completar el proceso de adquisición, preparación, entrenamiento, evaluación y pronóstico, la solución fue desplegada mediante un dashboard interactivo desarrollado en \textit{Streamlit}. Este dashboard permitió presentar los resultados del análisis y del modelo predictivo de forma visual, ordenada y accesible para el usuario final.

El despliegue integró tres elementos principales: la base de datos en \textit{Supabase}, el procesamiento en Python y la interfaz web en \textit{Streamlit}. \textit{Supabase} fue utilizado como repositorio central de datos, almacenando la información de usuarios y ventas. Python fue utilizado para realizar la limpieza de datos, generación de variables, entrenamiento de modelos, evaluación y pronóstico. \textit{Streamlit} fue utilizado para construir la interfaz visual mediante la cual el usuario puede interactuar con el sistema.

\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{4/despl_ima/d1.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d2.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		% \caption{Base de datos en Supabase}
		% \textit{Fuente: Elaboración propia}
		% \label{fig21}
	\end{center}	
\end{figure}
...
El sistema inicia con un formulario de acceso. El usuario ingresa sus credenciales y estas son validadas con la tabla de usuarios almacenada en \textit{Supabase}. Si los datos son correctos, el sistema identifica la sede asociada al usuario y carga únicamente la información correspondiente a dicha sede. Esta lógica permite implementar un control de acceso por sede, evitando que un usuario visualice información de otras unidades operativas.

\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		\includegraphics[width=0.75\textwidth]{4/despl_ima/d4.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		\caption{Control de acceso y base de datos en Supabase}
		\textit{Fuente: Elaboración propia}
		\label{fig22}
	\end{center}	
\end{figure}
...
Una vez dentro del dashboard, el usuario puede seleccionar la familia de productos que desea analizar. A partir de esta selección, el sistema filtra la información, prepara los datos y muestra diferentes secciones de análisis. Entre las principales funcionalidades se encuentran el resumen ejecutivo de ventas, el análisis por día de la semana, la participación porcentual diaria, el promedio mensual, el crecimiento anual, la serie histórica, la evaluación del modelo predictivo, la comparación entre ventas reales y predichas, y el pronóstico de ventas a 30 días.


\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{4/despl_ima/d8.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d6.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		\caption{Dashboard de análisis}
		\textit{Fuente: Elaboración propia}
		\label{fig23}
	\end{center}	
\end{figure}
...
El despliegue permite que el sistema funcione como una herramienta operativa para el administrador de cada sede. En lugar de trabajar únicamente con archivos manuales o cálculos aislados, el usuario puede acceder a una plataforma web, revisar el comportamiento histórico de las ventas y obtener una estimación futura que apoye la planificación del pedido a proveedores.
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{4/despl_ima/d7.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d4.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		\caption{Log in de Dashboard}
		\textit{Fuente: Elaboración propia}
		\label{fig24}
	\end{center}	
\end{figure}

Además, la integración con \textit{Supabase} permite que la información pueda actualizarse con nuevos registros de ventas. Conforme la empresa incorpore datos recientes, el sistema podrá procesarlos nuevamente y generar predicciones más actualizadas. Esto permite que el modelo mantenga una relación continua con la operación real de la empresa y no se limite a un análisis estático.

\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{4/despl_ima/d5.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d4.jpeg}
		% \includegraphics[width=0.6\textwidth]{4/despl_ima/d3.jpeg}
		\caption{Key de API 12222para conexión entre Supabase y Python}
		\textit{Fuente: Elaboración propia}
		\label{fig25}
	\end{center}	
\end{figure}

En conjunto, el despliegue de la solución permite transformar el modelo predictivo en una herramienta funcional, accesible y útil para la toma de decisiones. De esta manera, el proyecto no se limita a la construcción técnica de modelos, sino que culmina en una solución aplicada que permite mejorar la planificación de compras, reducir la incertidumbre en el reabastecimiento y apoyar una gestión más eficiente de productos perecibles.